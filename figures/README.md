...
| `2026-08-10_sweep_pipeline/` | Unified argparse pipeline outputs (`exp_*`), in subfolders `best_config/` (overview / groups / users at each model's best config), `pinned_a03g03/` (same three at (alpha,gamma)=(0.3,0.3)), `heatmaps/` ((alpha,gamma) grids); `exp_best_table.csv` at the root. `save_fig` routes reruns into the same subfolders | `scripts/plot_experiments.py` (default `--fig-dir` points here) |
| `2026-08-14_metric_summary/` | Discord-facing summaries of the sweep metric tables: severity-inversion bars (FedAVOT vs uniform, normalized to full) and standalone per-critical-group tail bars | `scripts/metric_summary_figures.py` (reads `sheets_export/*.csv`, no retraining) |
