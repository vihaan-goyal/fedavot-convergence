...
\renewcommand{\topfraction}{0.95}\renewcommand{\textfraction}{0.03}\setcounter{topnumber}{3}\setcounter{totalnumber}{4}
\begin{document}
...
Under \eqref{eq:constraints}, the estimator $g_t$ in \eqref{eq:gt} is conditionally unbiased:
\[
\mathbb{E}\!\left[g_t \,\big|\, \theta_t\right] \;=\; \nabla F(\theta_t).
\]
\end{lemma}
\begin{proof}
Condition on $\theta_t$, take expectation over $j_t\sim q$, and use $\sum_j q_j Y[i,j]=\mu_i$ from \eqref{eq:constraints}.
\end{proof}
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=0.56\textwidth]{adult_race_K3_4000rounds_decay.pdf}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.54\linewidth]{imdbwiki_infeasible_K3_4000rounds_decay.pdf}
  ...
  between the uniform average ($91.6$) and full coverage ($82.9$).}
...
to decide whether transport reweighting will help.
