...
for ax, tag in zip(axes, ["const", "decay1000"]):
    ...
    ax.invert_xaxis()
    ax.set_xticks(x)
    ax.set_xticklabels([f"$\\beta$={float(r['beta']):g}\n$\\nu$={float(r['infeasible_mass']):.2f}" for r in pts], fontsize=8)
    ax.set_title(PANEL[tag], fontsize=9, pad=6)
    ax.set_xlabel(r"sampling weights $\propto p^{\beta}$", fontsize=8.5)
    ax.tick_params(labelsize=8)
    ax.grid(alpha=0.3)
axes[0].set_ylabel(r"overall loss $F_p$ (CE)", fontsize=9)
axes[1].legend(fontsize=8, frameon=False, loc="upper left", handlelength=2.2)
