...
fig, axes = plt.subplots(1, 2, figsize=(7.2, 1.8), sharey=True)
for ax, tag in zip(axes, ["const", "decay1000"]):
    ...
    for r, xi in zip(pts, x):
        ax.annotate(rf"$\beta$={float(r['beta']):g}", (xi, float(r["meas_fedavot"])), textcoords="offset points",
                    xytext=(0, 5), ha="center", fontsize=7, color="0.3")
    ax.set_title(PANEL[tag], fontsize=8.5); ax.tick_params(labelsize=7.5)
